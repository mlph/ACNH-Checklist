...
export class ItemComponent extends BaseComponent<ItemJ> implements OnInit, OnDestroy {
  ...
  Sort(): ((a: ItemJ, b: ItemJ) => number) {
    ...
    switch (this.lastSort.active) {
      ...
      case 'series': return (p, q) => this.sorthelper_compare(p.seriesTranslations?.japanese, q.seriesTranslations?.japanese) * isAsc;
      case 'source': return (p, q) => this.sorthelper_compare_ItemSource(p.source, q.source) * isAsc;
      case 'catalog': return (p, q) => this.sorthelper_compare(p.catalog, q.catalog) * isAsc;
      default: return () => 0;
    }
  }
